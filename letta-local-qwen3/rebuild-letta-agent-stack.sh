...
[[ "$LETTA_MODEL" == ollama/* ]] ||
  die "LETTA_MODEL must be an Ollama Letta handle such as ollama/qwen3.5:9b-q4_K_M."

[[ "$LETTA_EMBEDDING_HANDLE" == ollama/* ]] ||
  die "LETTA_EMBEDDING_HANDLE must be an Ollama Letta handle such as ollama/nomic-embed-text:latest."
...
# ---------------------------------------------------------------------------
# Validate the local Ollama-backed model handles before deleting anything
# ---------------------------------------------------------------------------

log "Validating Letta connection and Ollama model handles..."
...
if ! printf '%s\n' "$LLM_MODELS_JSON" |
  ...
  printf '%s\n' 'Available Ollama LLM handles:' >&2
  printf '%s\n' "$LLM_MODELS_JSON" |
    jq -r '.[] | select((.handle // "") | startswith("ollama/")) | .handle' >&2
  exit 1
fi
...
if ! printf '%s\n' "$EMBEDDING_MODELS_JSON" |
  ...
  printf '%s\n' 'Available Ollama embedding handles:' >&2
  printf '%s\n' "$EMBEDDING_MODELS_JSON" |
    jq -r '.[] | select((.handle // "") | startswith("ollama/")) | .handle' >&2
  exit 1
fi

log "Ollama model handles are available through Letta."
...
# ---------------------------------------------------------------------------
# Create the local Ollama-backed Letta agent
# ---------------------------------------------------------------------------
...
printf '\n============================================================\n'
printf 'Letta local deployment completed successfully\n'
printf '============================================================\n\n'
